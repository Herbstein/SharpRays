namespace SharpRays.Hitables {
    ...
    internal class MovingSphere : IHitable {
        ...
        public bool BoundingBox(float t0, float t1, out AABB box) {
            box = AABB.SurroundingBox(new AABB(Center(Time0) - new Vector3(Radius),
                                               Center(Time0) + new Vector3(Radius)),
                                      new AABB(Center(Time1) - new Vector3(Radius),
                                               Center(Time1) + new Vector3(Radius)));
            return true;
        }
